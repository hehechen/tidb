...
// GetTiFlashPDPendingPeerInfo is a helper function calling `regions/check/down-peer|pending-peer`.
func GetTiFlashPDPendingPeerInfo(ctx context.Context, regionInfo *helper.RegionsInfo) error {
	is, err := getGlobalInfoSyncer()
	if err != nil {
		return errors.Trace(err)
	}
	return is.tiflashPlacementManager.GetPDPendingPeerInfo(ctx, regionInfo)
}
